...
<body class="relative bg-cover bg-center bg-fixed text-gray-800"
    ...
    <div x-data="bookingForm()" x-init="generateCalendar()" class="relative max-w-6xl mx-auto py-10 px-4 z-10">
        ...
        <div class="flex justify-between items-center mb-10">
            <div>
                ...
                <p class="text-gray-700">Hi, <b>{{ $user->name }}</b></p>
            </div>
            <a href="{{ route('bookings.index') }}" class="text-gray-600 hover:underline font-semibold">My Booking</a>
        </div>
        ...
        <div x-show="step === 1" x-cloak class="text-center">
            ...
            <div class="flex justify-center">
                <div class="p-8 bg-card rounded-2xl shadow card-hover max-w-sm">
                    ...
                    <p class="text-gray-700 text-sm">Jl. Cikalang Girang No.48 G, Tasikmalaya</p>
        ...
        <div x-show="step === 3" x-cloak class="text-center pt-16 relative">
            <button @click="step--" class="absolute left-0 top-0 bg-gray-800 text-white px-4 py-2 rounded-full">← Kembali</button>
            <h2 class="text-2xl mb-8">Pilih Service</h2>

            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
                <template x-for="s in services">
                    <div @click="chooseService(s.name)"
                        class="bg-card p-8 rounded-2xl cursor-pointer card-hover text-center">
                        <img :src="s.image" class="w-24 mx-auto">
                        <h3 class="mt-3 font-semibold text-xl" x-text="s.name"></h3>
                    </div>
                </template>
            </div>
        </div>

        <!-- STEP 4: Sub Service (FINAL FONT SIZE RESTORED) -->
        <div x-show="step === 4" x-cloak class="text-center relative pt-16">
            <button type="button" @click="step--"
                class="absolute left-0 top-0 bg-gray-700 text-white bg-opacity-80 hover:bg-gray-800 rounded-full px-4 py-2 font-semibold flex items-center shadow-lg">
            ...
            <div class="max-w-6xl mx-auto px-4">
                ...
                <!-- CENTER MODE ONLY WHEN service === Haircare & Extras -->
                <div class="grid gap-8"
                    :class="service === 'Haircare & Extras'
                ? 'justify-center max-w-xl mx-auto grid-cols-1 sm:grid-cols-2'
                : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3'">

                    <template x-for="(item, i) in (subServices[service] || []).filter(s => s && s.name)" :key="i">
                        <div @click="chooseSubService(item)"
                            class="cursor-pointer bg-card rounded-2xl shadow-lg p-10 card-hover transition-all border-2 flex flex-col items-center justify-center min-h-[260px]"
                            ...
                            <!-- SIZE RESTORED TO ORIGINAL BIG VERSION -->
        ...
        <div x-show="step === 6" x-cloak class="pt-16 relative max-w-4xl mx-auto text-center">
            ...
            <button @click="if(!date||!time){alert('Pilih tanggal & jam!');return;} step++"
                class="bg-gray-800 text-white mt-6 px-6 py-3 rounded-full">
                Lanjut
            </button>
        </div>
        ...
        <div x-show="step === 7" x-cloak class="max-w-md mx-auto text-center pt-16 relative">
            ...
            <div class="bg-card p-6 rounded-2xl shadow text-left">
            ...
            <form action="{{ route('book.store') }}" method="POST" class="mt-6">
                @csrf
                ...
                <button class="bg-green-600 text-white w-full py-3 rounded-full">Confirm Booking</button>
            </form>
        </div>
    ...
    <script>
        const AVAILABLE_TIMES_URL = @json(route('booking.available-times'));
        ...
        function bookingForm() {
            return {
                ...
                services: [{
                        name: "Haircut",
                        image: "{{ asset('images/services/haircut.png') }}"
                    },
                    {
                        name: "Chemical",
                        image: "{{ asset('images/services/chemical.png') }}"
                    },
                    {
                        name: "Haircare & Extras",
                        image: "{{ asset('images/services/haircare.png') }}"
                    },
                    {
                        name: "Colouring",
                        image: "{{ asset('images/services/coloring.png') }}"
                    },
                ],

                barbers: [{
                        name: "Wandi",
                        image: "{{ asset('images/barbers/wandi.png') }}"
                    },
                    {
                        name: "Fery",
                        image: "{{ asset('images/barbers/fery.png') }}"
                    },
                    {
                        name: "Ajay",
                        image: "{{ asset('images/barbers/ajay.png') }}"
                    },
                    {
                        name: "Sidik",
                        image: "{{ asset('images/barbers/sidik.png') }}"
                    },
                    {
                        name: "Anjas",
                        image: "{{ asset('images/barbers/anjas.jpeg') }}"
                    },
                    {
                        name: "Candra",
                        image: "{{ asset('images/barbers/chandra.png') }}"
                    },
                ],
                ...
                subServices: {
                    "Haircut": [{
                            ...
                            price: 60000,
                            image: "{{ asset('images/sub/premium.png') }}"
                        },
                        {
                            name: "Kids Haircut",
                            price: 50000,
                            image: "{{ asset('images/sub/kids.png') }}"
                        },
                        {
                            name: "Long Haircut",
                            price: 80000,
                            image: "{{ asset('images/sub/long.png') }}"
                        },
                    ],
                    "Chemical": [{
                            name: "Hair Perming",
                            price: 350000,
                            image: "{{ asset('images/sub/perming.png') }}"
                        },
                        {
                            name: "Smooth & Volumized",
                            price: 350000,
                            image: "{{ asset('images/sub/smooth.png') }}"
                        },
                        {
                            name: "Down Perm",
                            price: 150000,
                            image: "{{ asset('images/sub/downperm.png') }}"
                        },
                    ],
                    "Haircare & Extras": [{
                            name: "Hair Styling",
                            price: 20000,
                            image: "{{ asset('images/sub/styling.png') }}"
                        },
                        {
                            name: "Beard Trim",
                            price: 15000,
                            image: "{{ asset('images/sub/beard.png') }}"
                        },
                    ],
                    "Colouring": [{
                            name: "Black Hair Dye",
                            price: 60000,
                            image: "{{ asset('images/sub/blackdye.png') }}"
                        },
                        {
                            name: "Hair Coloring Full",
                            price: 300000,
                            image: "{{ asset('images/sub/fullcolor.png') }}"
                        },
                        {
                            name: "Highlight",
                            price: 250000,
                            image: "{{ asset('images/sub/highlight.png') }}"
                        },
                    ]
                },
                ...
                chooseSubService(item) {
                    this.sub_service = item.name;
